...
def add():
    if request.method == 'POST':
        ...
        urltest = checkifused(dest,request.form['dest_url'])
        if urltest == "1":
            return render_template("sorry.html") # mention URL is already in use. give the short code.
        else:
            g.db.execute('INSERT INTO urls (key, dest) values (?,?)',[newurlkey,request.form['dest_url']])
            g.db.commit()
